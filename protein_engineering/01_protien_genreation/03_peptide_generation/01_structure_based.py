...
try:
    ...
    antibody.click()
    # Navigate to structure based  peptide  generation section
    Protein= mywait.until(EC.presence_of_element_located((By.XPATH,"//span[normalize-space()='Structure Based peptide generation']")))
    Protein.click()
    
    Actions.send_keys(Keys.ESCAPE).perform()  # Close any pop-up if it appears

    # Verfication of navigation
    expected="https://app-boltzmann-com.web.app/protein-engineering/experiments/peptide-generation/structure-based-generation"
    # Check if the current URL matches the expected URL
    if driver.current_url == expected:
        print("Navigation to strucure based peptide generation section successful.")   
...
try:
    ...
    epi= mywait.until(EC.presence_of_element_located((By.XPATH, "//input[@id='epitope']")))
    epi.send_keys("LVLWGIHHP")
    seq= mywait.until(EC.presence_of_element_located((By.XPATH, "//input[@id='noofsequences']")))
    #backspace seq
