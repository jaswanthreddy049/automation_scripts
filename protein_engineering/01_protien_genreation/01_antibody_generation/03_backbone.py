...
try:
    ...
    Protein= mywait.until(EC.element_to_be_clickable((By.XPATH, "//span[normalize-space()='Backbone Generation']")))
    Protein.click()
    
    Actions.send_keys(Keys.ESCAPE).perform()  # Close any pop-up if it appears

    # Verfication of navigation
    expected="https://app-boltzmann-com.web.app/protein-engineering/experiments/anti-body-generation/backbone-generation"
...
try:
    for i in exp_list:
        ...
        # no.of backbone 
        backbone = mywait.until(EC.presence_of_element_located((By.XPATH, "//input[@id='noofbackbones']")))
        # Clear the input field before entering new value
        backbone.send_keys(Keys.CONTROL + 'a')
        backbone.send_keys(Keys.BACKSPACE)  # Press backspace to clear the input field
        # Enter the number of backbones
        backbone.send_keys("20")
