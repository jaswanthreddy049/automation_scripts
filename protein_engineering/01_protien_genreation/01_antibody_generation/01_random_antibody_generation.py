...
try:
    ...
    # Navigate to random antibody generation section
    Protein= mywait.until(EC.presence_of_element_located((By.XPATH, "//span[normalize-space()='Random Antibody Generation']")))
    Protein.click()
    
    Actions.send_keys(Keys.ESCAPE).perform()  # Close any pop-up if it appears

    # Verfication of navigation
    expected="https://app-boltzmann-com.web.app/protein-engineering/experiments/anti-body-generation/random-anti-body-generation"
    if driver.current_url == expected:
        print("Navigation to Random Antibody Generation section successful.")   
...
try:
    #Experiment name
    Experiment_name = "Random Antibody Generation"
