...
try:
    ...
    Protein.click()
    # Navigate to enzyme generation section
    antibody= mywait.until(EC.presence_of_element_located((By.XPATH, "//span[normalize-space()='Enzyme Generation']")))
    antibody.click()

    # Navigate to functional absed enzyme generation section
    Protein= mywait.until(EC.presence_of_element_located((By.XPATH, "//span[normalize-space()='Function-Based Enzyme Generation']")))
    Protein.click()
    
    Actions.send_keys(Keys.ESCAPE).perform()  # Close any pop-up if it appears

    # Verfication of navigation
    expected="https://app-boltzmann-com.web.app/protein-engineering/experiments/enzyme-generation/function-based-enzyme-generation"
    # Check if the current URL matches the expected URL
    if driver.current_url == expected:
        print("Navigation to functional based enzyme generation section successful.")   
...
try:
    ...
    ece= mywait.until(EC.presence_of_element_located((By.XPATH, "//input[@id='ecnumber']")))
     # Press backspace to clear the input field
    ece.send_keys("3.2.4.1")
    seq= mywait.until(EC.presence_of_element_located((By.XPATH, "//input[@id='noofsequence']")))
    #backspace seq
    seq.send_keys(Keys.COMMAND + 'a')
